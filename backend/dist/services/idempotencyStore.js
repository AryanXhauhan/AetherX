...
export class IdempotencyStore {
    ...
    static async check(key) {
        ...
        try {
            const pgResult = await query(`SELECT result_status, result_body FROM idempotency_keys 
         WHERE key = $1 AND expires_at > NOW()`, [key]);
            if (pgResult.rows.length > 0) {
                ...
                await redis.setex(`${REDIS_PREFIX}${key}`, DEFAULT_TTL_SECONDS, JSON.stringify(result));
                return result;
            }
        }
        catch (dbError) {
     ...
     */
    static async record(key, result, ttlSeconds = DEFAULT_TTL_SECONDS) {
        ...
        await Promise.allSettled([
            ...
            query(`INSERT INTO idempotency_keys (key, result_status, result_body, expires_at)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (key) DO NOTHING`, [key, result.status, JSON.stringify(result.body), expiresAt])
        ]);
    }
    /**
