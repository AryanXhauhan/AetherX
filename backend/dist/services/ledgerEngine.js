...
export class LedgerEngine {
     ...
     */
    static async getAccountLocked(client, ownerId, currency, type) {
        const res = await client.query(`SELECT id, balance, account_number 
       FROM accounts 
       WHERE owner_id = $1 AND currency = $2 AND account_type = $3 
       FOR UPDATE`, [ownerId, currency.toUpperCase(), type]);
     ...
     */
    static async lockEscrow(userId, symbol, side, price, quantity) {
        ...
        try {
            ...
            await client.query(`INSERT INTO transactions (id, reference, status, amount, currency, source_account_id, dest_account_id, description)
         VALUES ($1, $2, 'COMPLETED', $3, $4, $5, $6, $7)`, [transactionId, ref, amount, currency, checking.id, escrow.id, desc]);
            // ── 2. Create Double-Entry Legs ─────────────────────────
            // DEBIT checking (- balance)
            await client.query(`UPDATE accounts SET balance = balance - $1, updated_at = NOW() WHERE id = $2`, [amount, checking.id]);
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'DEBIT', $3, $4, $5)`, [transactionId, checking.id, amount, currency, `Debit checking wallet for escrow hold`]);
            // CREDIT escrow (+ balance)
            await client.query(`UPDATE accounts SET balance = balance + $1, updated_at = NOW() WHERE id = $2`, [amount, escrow.id]);
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5)`, [transactionId, escrow.id, amount, currency, `Credit escrow vault`]);
     ...
     */
    static async releaseEscrow(userId, symbol, side, price, quantity) {
        const client = await getClient();
        try {
            await client.query('BEGIN');
            const isBuy = side === 'BUY';
            ...
            await client.query(`INSERT INTO transactions (id, reference, status, amount, currency, source_account_id, dest_account_id, description)
         VALUES ($1, $2, 'COMPLETED', $3, $4, $5, $6, $7)`, [transactionId, ref, amount, currency, escrow.id, checking.id, desc]);
            // ── 2. Create Double-Entry Legs ─────────────────────────
            // DEBIT escrow (- balance)
            await client.query(`UPDATE accounts SET balance = balance - $1, updated_at = NOW() WHERE id = $2`, [amount, escrow.id]);
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'DEBIT', $3, $4, $5)`, [transactionId, escrow.id, amount, currency, `Debit escrow vault on release`]);
            // CREDIT checking (+ balance)
            await client.query(`UPDATE accounts SET balance = balance + $1, updated_at = NOW() WHERE id = $2`, [amount, checking.id]);
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5)`, [transactionId, checking.id, amount, currency, `Credit checking wallet on release`]);
            await client.query('COMMIT');
     ...
     */
    static async settleTrade(params) {
        ...
        try {
            ...
            await client.query(`INSERT INTO transactions (id, reference, status, amount, currency, source_account_id, dest_account_id, description)
         VALUES ($1, $2, 'COMPLETED', $3, 'USD', $4, $5, $6)`, [cashTxId, cashRef, cashAmount, buyerCashWallet.id, sellerCashWallet.id, cashDesc]);
            // Debit Buyer Cash Wallet
            await client.query(`UPDATE accounts SET balance = balance - $1 WHERE id = $2`, [cashAmount, buyerCashWallet.id]);
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'DEBIT', $3, 'USD', $4)`, [cashTxId, buyerCashWallet.id, cashAmount, `Debit Buyer USD wallet for trade execution`]);
            ...
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'CREDIT', $3, 'USD', $4)`, [cashTxId, sellerCashWallet.id, cashAmount, `Credit Seller USD checking wallet`]);
            ...
            await client.query(`INSERT INTO transactions (id, reference, status, amount, currency, source_account_id, dest_account_id, description)
         VALUES ($1, $2, 'COMPLETED', $3, $4, $5, $6, $7)`, [assetTxId, assetRef, assetAmount, asset, sellerAssetWallet.id, buyerAssetWallet.id, assetDesc]);
            ...
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'DEBIT', $3, $4, $5)`, [assetTxId, sellerAssetWallet.id, assetAmount, asset, `Debit Seller ${asset} wallet for trade execution`]);
            ...
            await client.query(`INSERT INTO ledger_entries (transaction_id, account_id, entry_type, amount, currency, description)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5)`, [assetTxId, buyerAssetWallet.id, assetAmount, asset, `Credit Buyer ${asset} checking wallet`]);
