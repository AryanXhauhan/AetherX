...
export class OMSEngine {
    static async init() {
        ...
        EventBus.subscribe('order.intents', 'oms-intent-processor', async (intent) => {
            await this.processIntent(intent);
        });
        // Command Handler for cancellations
        EventBus.subscribe('order.cancel_intents', 'oms-cancel-processor', async (intent) => {
            await this.processCancel(intent);
        });
        // 2. Event Projections (Events -> Read Models / State transitions)
        EventBus.subscribe('order.events', 'oms-state-projection', async (event) => {
     ...
     */
    static async processIntent(intent) {
        try {
            // 1. Pre-trade Risk Validation (Will check against in-memory position state)
            await RiskEngine.validateOrder(intent.userId, intent.symbol, intent.side, intent.type, intent.price || 0, intent.quantity);
     ...
     */
    static async processCancel(intent) {
        try {
            // Append cancel event
     ...
     */
    static async projectEvent(event) {
        const { aggregateId, eventType, payload } = event;
        try {
            switch (eventType) {
                case 'ORDER_CREATED':
                    await query(`INSERT INTO orders (id, user_id, symbol, side, type, price, quantity, status, order_subtype, time_in_force, stop_price)
             ...
             ON CONFLICT (id) DO NOTHING`, [
                    ...
                    if (payload.parentOrderId) {
                        await query(`INSERT INTO order_relationships (parent_order_id, child_order_id, relation_type)
               VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [payload.parentOrderId, aggregateId, payload.type]);
                    }
                    ...
                    if (!payload.parentOrderId) {
                        if (['MARKET', 'LIMIT'].includes(payload.type)) {
                            ...
                            if (activatedEvent)
                                await EventBus.publish('order.events', activatedEvent);
                        }
                        else {
                            ...
                            if (waitEvent)
                                await EventBus.publish('order.events', waitEvent);
                        }
                    }
                    break;
                ...
                case 'ORDER_TRIGGERED':
                    ...
                    if (triggeredActive)
                        await EventBus.publish('order.events', triggeredActive);
                    break;
                case 'ORDER_FILLED':
                    ...
                    for (const row of res.rows) {
                        ...
                        if (childTrigger)
                            await EventBus.publish('order.events', childTrigger);
                    }
                    break;
                case 'ORDER_CANCELED':
