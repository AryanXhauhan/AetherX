...
export class AnalyticsService {
     ...
     */
    static async getPerformanceStats(userId) {
        try {
            // Query trades made by this user
            // We will look at both buy and sell sides
            const tradesRes = await query(`SELECT t.price, t.quantity, t.side, o.type, t.executed_at, o.price as "orderPrice"
         FROM trades t
         JOIN orders o ON t.order_id = o.id
         WHERE o.user_id = $1
         ORDER BY t.executed_at ASC`, [userId]);
    ...
    static async getAssetAllocation(userId) {
        try {
            const res = await query(`SELECT currency, balance 
         FROM accounts 
         WHERE owner_id = $1 AND account_type = 'CHECKING'`, [userId]);
