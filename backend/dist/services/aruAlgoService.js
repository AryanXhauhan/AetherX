...
import { AruAlgo } from './aruAlgo.js';
export class AruAlgoService {
    static states = new Map();
    static subscribersActive = false;
    /**
     ...
     */
    static async start(intervalSec = 10) {
     ...
     */
    static seedInitialCandles(symbol, intervalSec) {
     ...
     */
    static bindTickListeners() {
        const symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT'];
        for (const symbol of symbols) {
            const topic = `market.ticks.${symbol.toLowerCase()}`;
            EventBus.subscribe(topic, `arualgo-processor-${symbol}`, async (tick) => {
                const state = this.states.get(symbol);
                if (!state)
                    return;
     ...
     */
    static getLatestIndicators(symbol) {
        const state = this.states.get(symbol);
        if (!state || !state.currentCandle)
            return null;
