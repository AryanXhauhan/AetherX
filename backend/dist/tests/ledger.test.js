...
const runTest = async () => {
    ...
    try {
        ...
        // Create Buyer
        await dbClient.query(`INSERT INTO users (id, email, password_hash) VALUES ($1, $2, $3)`, [buyerId, `buyer-${buyerId.substring(0, 6)}@test.com`, 'test_hash']);
        // Create USD Checking Account
        const buyerUsdAcc = `ACC-${buyerId.substring(0, 8)}-USD`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'USD', 100000.0, 'CHECKING')`, [uuidv4(), buyerId, buyerUsdAcc]);
        // Create BTC Checking Account
        const buyerBtcAcc = `ACC-${buyerId.substring(0, 8)}-BTC`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'BTC', 0.0, 'CHECKING')`, [uuidv4(), buyerId, buyerBtcAcc]);
        // Create USD Escrow Account
        const buyerUsdEscrow = `ACC-${buyerId.substring(0, 8)}-USD-ESCROW`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'USD', 0.0, 'ESCROW')`, [uuidv4(), buyerId, buyerUsdEscrow]);
        // Create Seller
        await dbClient.query(`INSERT INTO users (id, email, password_hash) VALUES ($1, $2, $3)`, [sellerId, `seller-${sellerId.substring(0, 6)}@test.com`, 'test_hash']);
        // Create USD Checking Account
        const sellerUsdAcc = `ACC-${sellerId.substring(0, 8)}-USD`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'USD', 0.0, 'CHECKING')`, [uuidv4(), sellerId, sellerUsdAcc]);
        // Create BTC Checking Account (Seeded with 2.0 BTC to sell!)
        const sellerBtcAcc = `ACC-${sellerId.substring(0, 8)}-BTC`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'BTC', 2.0, 'CHECKING')`, [uuidv4(), sellerId, sellerBtcAcc]);
        // Create BTC Escrow Account
        const sellerBtcEscrow = `ACC-${sellerId.substring(0, 8)}-BTC-ESCROW`;
        await dbClient.query(`INSERT INTO accounts (id, owner_id, account_number, currency, balance, account_type) VALUES ($1, $2, $3, 'BTC', 0.0, 'ESCROW')`, [uuidv4(), sellerId, sellerBtcEscrow]);
        ...
        // Insert orders in DB
        await query(`INSERT INTO orders (id, user_id, symbol, side, type, price, quantity, filled_quantity, status)
       VALUES ($1, $2, 'BTCUSDT', 'BUY', 'LIMIT', $3, $4, 0.0, 'PENDING')`, [buyOrderId, buyerId, price, qty]);
        await query(`INSERT INTO orders (id, user_id, symbol, side, type, price, quantity, filled_quantity, status)
       VALUES ($1, $2, 'BTCUSDT', 'SELL', 'LIMIT', $3, $4, 0.0, 'PENDING')`, [sellOrderId, sellerId, price, qty]);
        const buyOrder = {
            id: buyOrderId,
            userId: buyerId,
            symbol: 'BTCUSDT',
        ...
        const entriesRes = await query(`SELECT le.entry_type, le.amount, le.currency, t.reference
