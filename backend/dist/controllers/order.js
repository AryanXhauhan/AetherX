...
import { handleCreateOrderIntent } from '../commands/createOrderIntentHandler.js';
import { handleCancelOrderIntent } from '../commands/cancelOrderIntentHandler.js';
...
export const placeOrder = async (req, res) => {
    ...
    try {
        ...
        // Use command handler to process order intent with validation and event sourcing
        await handleCreateOrderIntent(intent);
        res.status(202).json({
...
export const cancelOrder = async (req, res) => {
    ...
    try {
        ...
        const orderRes = await query(`SELECT symbol, status
       FROM orders
       WHERE id = $1 AND user_id = $2`, [orderId, userId]);
        ...
        // Use command handler to process cancel intent with validation and event sourcing
        await handleCancelOrderIntent(intent);
...
export const getOrders = async (req, res) => {
    ...
    try {
        const activeRes = await query(`SELECT id, symbol, side, type, price, quantity, filled_quantity as "filledQuantity", status, created_at as "createdAt"
       FROM orders
       WHERE user_id = $1 AND status IN ('PENDING', 'PARTIALLY_FILLED', 'TRIGGER_WAITING')
       ORDER BY created_at DESC`, [userId]);
        const historyRes = await query(`SELECT id, symbol, side, type, price, quantity, filled_quantity as "filledQuantity", status, created_at as "createdAt"
       FROM orders
