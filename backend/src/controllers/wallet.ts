...
export const getBalances = async (req: Request, res: Response) => {
  ...
  try {
    const dbRes = await query(
      `SELECT id, currency, balance, account_type as "type", account_number as "accountNumber"
       FROM accounts 
       WHERE owner_id = $1 
...
export const getPositions = async (req: Request, res: Response) => {
  ...
  try {
    const dbRes = await query(
      `SELECT id, symbol, size, average_entry_price as "averageEntryPrice"
       FROM positions 
       WHERE user_id = $1 AND size != 0.00000000`,
      [userId]
    );
