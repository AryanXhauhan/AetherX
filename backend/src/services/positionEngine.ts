...
export interface PositionState {
  ...
  leverage: number;
  isDirty: boolean; // Flag for DB sync
}

export class PositionEngine {
  private static positions: Map<string, PositionState> = new Map();
  ...
  public static async init() {
    ...
    await EventBus.subscribe('order.events', 'position-engine-trades', async (event: any) => {
    ...
    await EventBus.subscribe('market.ticks', 'position-engine-mtm', async (tick: any) => {
  ...
  public static getPosition(userId: string, symbol: string): PositionState | undefined {
    return this.positions.get(`${userId}-${symbol}`);
  }

  public static getUserPositions(userId: string): PositionState[] {
   ...
   */
  private static handleTradeExecution(trade: any) {
  ...
  private static processSide(userId: string, symbol: string, price: number, quantity: number, side: 'BUY' | 'SELL') {
    ...
    let liqPrice = null;
    if (newSize > 0) liqPrice = newAvgEntry * (1 - 1 / leverage);
    if (newSize < 0) liqPrice = newAvgEntry * (1 + 1 / leverage);
   ...
   */
  private static markToMarket(symbol: string, currentPrice: number) {
   ...
   */
  private static async flushToDB() {
    const dirtyPositions = Array.from(this.positions.values()).filter(p => p.isDirty);
    if (dirtyPositions.length === 0) return;

    try {
      await query('BEGIN');
      for (const pos of dirtyPositions) {
        await query(
          `INSERT INTO positions (user_id, symbol, size, average_entry_price, realized_pnl, unrealized_pnl, margin_used, liquidation_price, leverage, updated_at)
           ...
           ON CONFLICT (user_id, symbol) DO UPDATE SET
             ...
             updated_at = NOW()`,
          [pos.userId, pos.symbol, pos.size, pos.averageEntryPrice, pos.realizedPnl, pos.unrealizedPnl, pos.marginUsed, pos.liquidationPrice, pos.leverage]
        );
        pos.isDirty = false;
      }
      await query('COMMIT');
