...
export class TriggerEngine {
  ...
  private static stopBuckets: Map<string, TriggerOrder[]> = new Map();
  private static takeProfitBuckets: Map<string, TriggerOrder[]> = new Map();
  ...
  public static async init() {
    ...
    await EventBus.subscribe('market.ticks', 'trigger-engine-evaluator', async (tick: any) => {
      await this.evaluateTriggers(tick.symbol, tick.price);
    });

    // Listen to order events to dynamically add/remove triggers
    await EventBus.subscribe('order.events', 'trigger-engine-manager', async (event: any) => {
  ...
  private static async loadActiveTriggers() {
    const res = await query(
      `SELECT id, symbol, side, type, stop_price as "triggerPrice"
       FROM orders
  ...
  private static addTrigger(order: any) {
    if (!order.triggerPrice && !order.stopPrice) return;
  ...
  private static removeTrigger(orderId: string, symbol: string) {
  ...
  private static async evaluateTriggers(symbol: string, currentPrice: number) {
     const stops = this.stopBuckets.get(symbol) || [];
     const takeProfits = this.takeProfitBuckets.get(symbol) || [];

     const triggered: TriggerOrder[] = [];
