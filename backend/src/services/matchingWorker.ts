...
class OrderQueue {
  ...
  push(order: OrderNode) {
    if (!this.head) {
      this.head = order;
      this.tail = order;
    } else {
      order.prev = this.tail;
      if (this.tail) this.tail.next = order;
      this.tail = order;
    }
  ...
  remove(order: OrderNode) {
    ...
    else this.head = order.next;

    if (order.next) order.next.prev = order.prev;
    else this.tail = order.prev;
...
class OrderBook {
  symbol: string;
  bids: Map<number, OrderQueue> = new Map(); // price -> Queue
  asks: Map<number, OrderQueue> = new Map(); // price -> Queue
  orderMap: Map<string, { node: OrderNode, price: number }> = new Map(); // orderId -> Node
  ...
  bidPrices: number[] = []; // descending
  askPrices: number[] = []; // ascending
  ...
  private insertResting(node: OrderNode, levels: Map<number, OrderQueue>, prices: number[], sortFn: (a: number, b: number) => number) {
  ...
  private matchOrder(taker: OrderNode, makerPrices: number[], makerLevels: Map<number, OrderQueue>, isBuy: boolean) {
    let i = 0;
    while (i < makerPrices.length && taker.quantity > 0) {
      ...
      if (taker.price > 0) { // If not market order
        if (isBuy && taker.price < bestPrice) break;
        if (!isBuy && taker.price > bestPrice) break;
      }
...
if (parentPort) {
  parentPort.on('message', (msg: any) => {
    try {
      ...
      switch (type) {
        case 'ADD_ORDER':
          if (book) book.addOrder(payload);
          break;
        case 'CANCEL_ORDER':
          if (book) book.cancelOrder(payload.id);
          break;
        case 'GET_DEPTH':
          if (book) book['broadcastDepth']();
          break;
      }
    } catch (err) {
