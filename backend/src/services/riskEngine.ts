...
import { PositionEngine, PositionState } from './positionEngine.js';
...
export class RiskEngine {
   ...
   */
  public static async validateOrder(userId: string, symbol: string, side: 'BUY' | 'SELL', type: string, price: number, quantity: number): Promise<boolean> {
    ...
    try {
      ...
      const accountRes = await query(
        `SELECT balance FROM accounts WHERE owner_id = $1 AND currency = 'USD'`,
        [userId]
      );
   ...
   */
  public static async evaluateDrawdown(userId: string): Promise<{ liquidated: boolean; message: string }> {
