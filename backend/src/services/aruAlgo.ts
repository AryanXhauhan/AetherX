...
export interface SignalLabel {
  type: 'primaryBuy' | 'primarySell' | 'simpleBuy' | 'simpleSell';
  time: number;
  price: number;
  sl: number | null;
  tp: number | null;
...
export interface AruAlgoResult {
  ready: boolean;
  time?: number;
  close?: number;
  atr?: number;
  ...
  buyCond?: boolean;
  sellCond?: boolean;
  simpleBuyCond?: boolean;
  simpleSellCond?: boolean;
  lastSL?: number | null;
  lastTP?: number | null;
  signalLabels?: SignalLabel[];
  barColor?: 'buy' | 'sell' | null;
}

export class AruAlgo {
  public sensitivity: number;
  public atrPeriod: number;
  public trendEmaPeriod: number;
  public rsiPeriod: number;
  public rsiOverbought: number;
  public rsiOversold: number;
  public adxPeriod: number;
  public adxThreshold: number;
  public slMultiplier: number;
  public tpMultiplier: number;

  private _candles: Candle[] = [];
  private _maxLen: number;

  // Previous historical state references
  private atrStopPrev: number | null = null;
  private lastSL: number = NaN;
  private lastTP: number = NaN;
  private _lastSmoothedAtrStop: number | null = null;

  // Memoized mathematical indicators caching
  private _emaCache: Record<string, number> = {};
  private _rsiCache: { prevAvgGain: number | null; prevAvgLoss: number | null } = {
    prevAvgGain: null,
    prevAvgLoss: null
  };
  
  private _plusDM_rma: number | null = null;
  private _minusDM_rma: number | null = null;
  private _tr_rma: number | null = null;
  private _dx_rma: number | null = null;

  constructor(params: Partial<AruAlgo> = {}) {
  ...
  private _pushCandle(c: Candle): void {
  ...
  private _ema(period: number, prevEma: number | null, value: number): number {
    const k = 2 / (period + 1);
  ...
  private _rma(period: number, prevRma: number | null, value: number): number {
  ...
  private _computeATRUsingRMA(): number | null {
  ...
  private _trueRange(prev: Candle, curr: Candle): number {
  ...
  private _updateRSI(): number | null {
  ...
  private _updateADX(): number | null {
    const n = this._candles.length;
  ...
  private _updateEMA(period: number): number | null {
    const n = this._candles.length;
    if (n < 1) return null;
  ...
  public processCandle(candle: Omit<Candle, 'time'> & { time: number | string }): AruAlgoResult {
    ...
    const signalLabels: SignalLabel[] = [];
  ...
  private _wasCrossOver(aNow: number, bNow: number): boolean {
    const n = this._candles.length;
    if (n < 2) return false;
  ...
  private _round(v: number): number | null {
